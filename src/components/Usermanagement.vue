		 ...
		 <el-dialog title="新增数据" :visible.sync="dialogFormVisibles">
		   <el-form>
		    ...
		    <el-input v-model="password" placeholder="请输入密码">
		 	...
		 	<el-input v-model="emial" placeholder="请输入邮箱">
		 	</el-input>
		 	</el-form-item>
		   </el-form>
	...
	export default{
		...
		methods:{
			...
			add(){
				...
				if(this.username!=''&&this.password!=''&&this.emial!=''){
				axios.post("http://43.142.24.86:8890/api/InsertUser",{
			...
			update(){
				...
				if(this.username!=''&&this.password!=''&&this.emial!=''){
				axios.post("http://43.142.24.86:8890/api/updateUser",{
					'id':this.id,
					'username':this.username,
					'password':this.password,	
					'emial':this.emial,
			...
			sousou(){
				if(this.input==''){
					 this.$message.error('查询不能为空！请输入ID或用户名');
				}
				if(this.input!=''){
					axios.get("http://43.142.24.86:8890/api/selectByIdUser",{
						params:{
							'id':this.input,		}
						}).then(by=>{
						  let arr=[]
						  arr.push(by.data.data)
						  this.pagelist=arr  
					})
					
					axios.get("http://43.142.24.86:8890/api/selectByUsername",{
						params:{
							'username':this.input
						}
					}).then(username=>{
						this.pagelist=username.data.data
					})
				}/* else{
			...
			findBy(){
			if(this.inputs==''){
				this.$message.error('查询邮箱不能为空，请输入!');
			}
			if(this.inputs!=''){
				axios.get("http://43.142.24.86:8890/api/selectByEmial",{				
						params:{
							'emial':this.inputs
						}
				}).then(findbyEmial=>{				
					this.pagelist=findbyEmial.data.data
		...
		mounted() {
			/* 分页查询 */
			axios.get("http://43.142.24.86:8890/api/pageUser",{
				params:{
					'id':this.page,	 
				}
			}).then(page=>{
				this.pagelist=page.data.data
			})
			/* 查询全部 */
